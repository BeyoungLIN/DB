...
def run_predict(folder_ip_pth, folder_op_pth):
    os.system('export CUDA_VISIBLE_DEVICES=2')
    os.system(
        'python special_demo.py experiments/seg_detector/fakepages_resnet50_deform_thre.yaml --image_path ' + folder_ip_pth + '/ --visualize --result_dir ' + folder_op_pth + '/ --resume models/fakepage_res50_iter2.bin --box_thresh 0.5 --sort_boxes')
    ...
    if os.path.isdir(folder_pth):
        ...
        for file in files:
            if os.path.splitext(file)[1].lower() in IMG_EXT:  # 读取所有图片文件
                # try:
                #  读res
                file_path = os.path.join(folder_pth, file)  # 原文件夹内图片路径
                txt_path = os.path.join(folder_pth, 'res_' + file.split('.')[0] + '.txt')  # 图片对应的txt名
